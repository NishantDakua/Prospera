  ...
  try {
    ...
    return NextResponse.json({
      kyc: {
        ...
        state: row.state,
        pincode: row.pincode,
        aadhaarNumber: row.aadhaar_number ? `XXXX-XXXX-${row.aadhaar_number.slice(-4)}` : null,
        panNumber: row.pan_number ? `XXXXX${row.pan_number.slice(-5)}` : null,
        bankName: row.bank_name,
        bankAccount: row.bank_account ? `XXXXXXX${row.bank_account.slice(-4)}` : null,
